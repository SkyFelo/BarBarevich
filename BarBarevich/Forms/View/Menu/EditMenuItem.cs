...
namespace BarBarevich.Forms.View.Menu
{
    public partial class EditMenuItem : Form
    {
        ...
        public EditMenuItem(MenuView lastForm, string id, string name, 
            string category, string price, string grams, string calories)
        {
            InitializeComponent();
            ...
            comboBoxValue = category;
            textBoxGrams.Text = grams;
            textBoxCalories.Text = calories;
            textBoxPrice.Text = price.Split(',')[0]; 
        }
        ...
        private void EditMenuItem_Load(object sender, EventArgs e)
        {
            ...
            comboBoxCategory.SelectedItem = comboBoxValue;
            ...
            foreach (var row in productClass.GetAllProductsSorted().Rows.Cast<DataRow>())
            {
                ...
                int menuId = Convert.ToInt32(row["id_product"]);
                comboBoxProduct.Items.Add(new ComboBoxItem($"{name} ({unit})", menuId));
            }
            ...
            DataTable recipeTable = productClass.GetRecipeByMenuId(id);
            foreach (DataRow row in recipeTable.Rows)
            {
                int id_product = Convert.ToInt32(row["id_product"]);
                string product_name = row["product_name"].ToString();
                double quantity = Convert.ToDouble(row["quantity"]);
                recipe.Add((id_product, product_name, quantity));
                dataGridViewRecipe.Rows.Add(product_name, quantity);
            }

            UpdateTotalGramsFromRecipe();
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"Пункт меню {name} уже существует.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!menuClass.EditMenuItem(id, name, id_category,
    ...
    calories.ToString(System.Globalization.CultureInfo.InvariantCulture)))
            ...
            {
                MessageBox.Show("Ошибка при обновлении информации о пункте меню.");
                return;
            }

            productClass.ClearRecipeForMenu(id);
            foreach (var entry in recipe)
                productClass.AddProductToRecipe(entry.id_product, int.Parse(id), entry.quantity);

            MessageBox.Show("Информация о пункте меню успешно обновлена.");
        ...
        private void UpdateTotalGramsFromRecipe()
        {
            double totalGrams = 0;
            ...
            foreach (DataGridViewRow row in dataGridViewRecipe.Rows)
            {
                if (row.Cells[1].Value != null &&
                    double.TryParse(row.Cells[1].Value.ToString(), out double qty))
                {
                    totalGrams += qty;
                }
            }

            textBoxGrams.Text = totalGrams.ToString("0.##");
        }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (keyData == Keys.F1)
            {
                HelpManager.ShowHelp(nameof(EditMenuItem));
                return true;
            }
